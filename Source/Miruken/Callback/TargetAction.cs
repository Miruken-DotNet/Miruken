namespace Miruken.Callback
{
    ...
    public delegate bool TargetAction<in T>(T target, IHandler handler);
    ...
    public class TargetActionBuilder<T, R>
    {
        ...
        public Tuple<R, Maybe<S>> Invoke<S>(Func<T, S> block)
        {
            ...
            return Tuple.Create(_notify((t, _) =>
            {
                s = block(t);
                return true;
            }), s);
        ...
        {
            return _notify((t, handler) =>
            {
                var args = handler.ResolveArgs(Arguments(block));
        ...
        {
            var s = Maybe<S>.Nothing;
            return Tuple.Create(_notify((t, handler) =>
            {
                var args = handler.ResolveArgs(Arguments(block));
                if (args == null) return false;
                s = block(t, (A)args[0]);
