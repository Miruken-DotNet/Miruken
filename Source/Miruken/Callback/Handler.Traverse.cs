...
{
    public static IHandlerAxis Axis(this IHandlerAxis handler, TraversingAxis axis) =>
        handler == null ? null : new HandlerAxis(handler, axis);

    public static IHandler Publish(this IHandlerAxis handler) =>
        handler.SelfOrDescendant().Notify();
