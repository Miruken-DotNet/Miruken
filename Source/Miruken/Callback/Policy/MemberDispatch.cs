...
public abstract class MemberDispatch
{
        ...
        IEnumerable<Attribute> attributes = null)
    {
        ...
        if (member.ReflectedType is {} owner)
        {
            var attribs = Attribute.GetCustomAttributes(owner, true).Normalize();
            if (attribs is { Length: > 0 })
                Attributes = Attributes.Concat(attribs);
        }
    ...
    public abstract object Invoke(object target, object[] args, Type returnType = null);

    public MemberPipeline GetPipeline(Type callbackType) =>
        MemberPipeline.GetPipeline(callbackType, LogicalReturnType);
    ...
    private Type ConfigureMember()
    {
        ...
        return task != null ? task.GetGenericArguments()[0] : typeof(object);
    }
